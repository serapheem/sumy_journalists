	...
	<table>
		<thead>
			<tr>
				...
				<th width="20">&nbsp;</th>
				<th class="tl"><b><?php echo Yii::t( 'main', 'NAME' ) ?></b></th>
		...
		<?php else : ?>
			<?php foreach ( $rows AS $k => $row ) : ?>
				...
				if ( $row->top10 ) 
				{
					$top10_title = Yii::t( 'main', 'UNPUBLISH' );
					$top10_class = 'state featured';
				} 
				else {
					$top10_title = Yii::t( 'main', 'PUBLISH' );
					$top10_class = 'state unfeatured';
				}
				$top10_onclick = "postSend('/admin/{$model_name}/edit', { id: {$row->id}, '{$this->model}[top10]': " 
								. (1 - $row->top10) . " }); return false;";
				// Get order data
